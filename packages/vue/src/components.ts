/* eslint-disable */
/* tslint:disable */
/* auto-generated vue proxies */
import { defineContainer } from './vue-component-lib/utils';

import type { JSX } from '@go-ui/core/components';

import { defineCustomElement as defineGoAccordion } from '@go-ui/core/components/go-accordion.js';
import { defineCustomElement as defineGoAccordionItem } from '@go-ui/core/components/go-accordion-item.js';
import { defineCustomElement as defineGoBadge } from '@go-ui/core/components/go-badge.js';
import { defineCustomElement as defineGoBanner } from '@go-ui/core/components/go-banner.js';
import { defineCustomElement as defineGoBlockquote } from '@go-ui/core/components/go-blockquote.js';
import { defineCustomElement as defineGoBreadcrumb } from '@go-ui/core/components/go-breadcrumb.js';
import { defineCustomElement as defineGoButton } from '@go-ui/core/components/go-button.js';
import { defineCustomElement as defineGoButtonGroup } from '@go-ui/core/components/go-button-group.js';
import { defineCustomElement as defineGoCalloutSection } from '@go-ui/core/components/go-callout-section.js';
import { defineCustomElement as defineGoCard } from '@go-ui/core/components/go-card.js';
import { defineCustomElement as defineGoCardRow } from '@go-ui/core/components/go-card-row.js';
import { defineCustomElement as defineGoCheckbox } from '@go-ui/core/components/go-checkbox.js';
import { defineCustomElement as defineGoChip } from '@go-ui/core/components/go-chip.js';
import { defineCustomElement as defineGoContent } from '@go-ui/core/components/go-content.js';
import { defineCustomElement as defineGoContentLayout } from '@go-ui/core/components/go-content-layout.js';
import { defineCustomElement as defineGoDatepicker } from '@go-ui/core/components/go-datepicker.js';
import { defineCustomElement as defineGoDialog } from '@go-ui/core/components/go-dialog.js';
import { defineCustomElement as defineGoDropdown } from '@go-ui/core/components/go-dropdown.js';
import { defineCustomElement as defineGoDropdownItem } from '@go-ui/core/components/go-dropdown-item.js';
import { defineCustomElement as defineGoDropdownMenu } from '@go-ui/core/components/go-dropdown-menu.js';
import { defineCustomElement as defineGoDropdownSeparator } from '@go-ui/core/components/go-dropdown-separator.js';
import { defineCustomElement as defineGoField } from '@go-ui/core/components/go-field.js';
import { defineCustomElement as defineGoFieldset } from '@go-ui/core/components/go-fieldset.js';
import { defineCustomElement as defineGoFooter } from '@go-ui/core/components/go-footer.js';
import { defineCustomElement as defineGoGovAuLogo } from '@go-ui/core/components/go-gov-au-logo.js';
import { defineCustomElement as defineGoHeaderBar } from '@go-ui/core/components/go-header-bar.js';
import { defineCustomElement as defineGoHeadingRow } from '@go-ui/core/components/go-heading-row.js';
import { defineCustomElement as defineGoHero } from '@go-ui/core/components/go-hero.js';
import { defineCustomElement as defineGoIcon } from '@go-ui/core/components/go-icon.js';
import { defineCustomElement as defineGoInput } from '@go-ui/core/components/go-input.js';
import { defineCustomElement as defineGoLink } from '@go-ui/core/components/go-link.js';
import { defineCustomElement as defineGoMainNav } from '@go-ui/core/components/go-main-nav.js';
import { defineCustomElement as defineGoMd } from '@go-ui/core/components/go-md.js';
import { defineCustomElement as defineGoNavDrawer } from '@go-ui/core/components/go-nav-drawer.js';
import { defineCustomElement as defineGoNavLink } from '@go-ui/core/components/go-nav-link.js';
import { defineCustomElement as defineGoNavList } from '@go-ui/core/components/go-nav-list.js';
import { defineCustomElement as defineGoOverlay } from '@go-ui/core/components/go-overlay.js';
import { defineCustomElement as defineGoProgress } from '@go-ui/core/components/go-progress.js';
import { defineCustomElement as defineGoRadio } from '@go-ui/core/components/go-radio.js';
import { defineCustomElement as defineGoSearchBar } from '@go-ui/core/components/go-search-bar.js';
import { defineCustomElement as defineGoSelect } from '@go-ui/core/components/go-select.js';
import { defineCustomElement as defineGoSkipLink } from '@go-ui/core/components/go-skip-link.js';
import { defineCustomElement as defineGoSpinner } from '@go-ui/core/components/go-spinner.js';
import { defineCustomElement as defineGoSwitch } from '@go-ui/core/components/go-switch.js';
import { defineCustomElement as defineGoTab } from '@go-ui/core/components/go-tab.js';
import { defineCustomElement as defineGoTableWrapper } from '@go-ui/core/components/go-table-wrapper.js';
import { defineCustomElement as defineGoTabs } from '@go-ui/core/components/go-tabs.js';
import { defineCustomElement as defineGoTextarea } from '@go-ui/core/components/go-textarea.js';
import { defineCustomElement as defineGoToTop } from '@go-ui/core/components/go-to-top.js';
import { defineCustomElement as defineGoToc } from '@go-ui/core/components/go-toc.js';
import { defineCustomElement as defineGoTooltip } from '@go-ui/core/components/go-tooltip.js';


export const GoAccordion = /*@__PURE__*/ defineContainer<JSX.GoAccordion>('go-accordion', defineGoAccordion, [
  'multiple'
]);


export const GoAccordionItem = /*@__PURE__*/ defineContainer<JSX.GoAccordionItem>('go-accordion-item', defineGoAccordionItem, [
  'heading',
  'headingTag',
  'autoHeight',
  'active',
  'opened',
  'opening',
  'closed',
  'closing'
]);


export const GoBadge = /*@__PURE__*/ defineContainer<JSX.GoBadge>('go-badge', defineGoBadge, [
  'count',
  'min',
  'max',
  'label',
  'dotOnly'
]);


export const GoBanner = /*@__PURE__*/ defineContainer<JSX.GoBanner>('go-banner', defineGoBanner, [
  'variant',
  'heading',
  'dismissible',
  'dismissed'
]);


export const GoBlockquote = /*@__PURE__*/ defineContainer<JSX.GoBlockquote>('go-blockquote', defineGoBlockquote, [
  'citeUrl',
  'hideQuoteMark'
]);


export const GoBreadcrumb = /*@__PURE__*/ defineContainer<JSX.GoBreadcrumb>('go-breadcrumb', defineGoBreadcrumb, [
  'label',
  'items',
  'hideCurrent'
]);


export const GoButton = /*@__PURE__*/ defineContainer<JSX.GoButton>('go-button', defineGoButton, [
  'type',
  'disabled',
  'variant',
  'block',
  'outline',
  'outlineFill',
  'flat',
  'round',
  'icon',
  'stack',
  'compact',
  'href'
]);


export const GoButtonGroup = /*@__PURE__*/ defineContainer<JSX.GoButtonGroup>('go-button-group', defineGoButtonGroup, [
  'block',
  'connected'
]);


export const GoCalloutSection = /*@__PURE__*/ defineContainer<JSX.GoCalloutSection>('go-callout-section', defineGoCalloutSection, [
  'heading',
  'headingTag',
  'description',
  'imgSrc',
  'imgAlt',
  'imgPos'
]);


export const GoCard = /*@__PURE__*/ defineContainer<JSX.GoCard>('go-card', defineGoCard, [
  'cardTitle',
  'cardSubtitle',
  'mediaPosition',
  'href',
  'target',
  'flat',
  'border'
]);


export const GoCardRow = /*@__PURE__*/ defineContainer<JSX.GoCardRow>('go-card-row', defineGoCardRow, [
  'noStretch',
  'cols',
  'colsTablet',
  'colsDesktop',
  'colsLarge',
  'stagger',
  'heading',
  'headingTag',
  'moreLinkHref',
  'moreLinkText'
]);


export const GoCheckbox = /*@__PURE__*/ defineContainer<JSX.GoCheckbox, JSX.GoCheckbox["checked"]>('go-checkbox', defineGoCheckbox, [
  'checked',
  'indeterminate',
  'name',
  'disabled',
  'value',
  'label',
  'hint',
  'error',
  'hintId',
  'errorId'
],
'checked', 'change');


export const GoChip = /*@__PURE__*/ defineContainer<JSX.GoChip>('go-chip', defineGoChip, [
  'variant',
  'outline',
  'clickable',
  'dismissible',
  'chipClick',
  'chipDismissed'
]);


export const GoContent = /*@__PURE__*/ defineContainer<JSX.GoContent>('go-content', defineGoContent);


export const GoContentLayout = /*@__PURE__*/ defineContainer<JSX.GoContentLayout>('go-content-layout', defineGoContentLayout, [
  'pageHeading',
  'preHeading',
  'intro',
  'breadcrumbs',
  'heroImgSrc',
  'heroImgAlt',
  'noHero',
  'toc',
  'tocProps',
  'sidebarMobilePosition',
  'sidebarDesktopPosition',
  'sidebarSticky'
]);


export const GoDatepicker = /*@__PURE__*/ defineContainer<JSX.GoDatepicker, JSX.GoDatepicker["value"]>('go-datepicker', defineGoDatepicker, [
  'labelId',
  'prefixId',
  'suffixId',
  'hintId',
  'errorId',
  'name',
  'label',
  'disabled',
  'hint',
  'error',
  'readonly',
  'value',
  'options',
  'goChange'
],
'value', 'v-go-change', 'goChange');


export const GoDialog = /*@__PURE__*/ defineContainer<JSX.GoDialog>('go-dialog', defineGoDialog, [
  'active',
  'persistent',
  'heading'
]);


export const GoDropdown = /*@__PURE__*/ defineContainer<JSX.GoDropdown>('go-dropdown', defineGoDropdown, [
  'triggerSelector',
  'isActive',
  'width',
  'noTriggerClickHandler',
  'fullWidth',
  'disabled',
  'opened',
  'closed'
]);


export const GoDropdownItem = /*@__PURE__*/ defineContainer<JSX.GoDropdownItem>('go-dropdown-item', defineGoDropdownItem, [
  'width',
  'disabled',
  'selected'
]);


export const GoDropdownMenu = /*@__PURE__*/ defineContainer<JSX.GoDropdownMenu>('go-dropdown-menu', defineGoDropdownMenu, [
  'triggerSelector',
  'isActive',
  'width',
  'menuId',
  'persistent'
]);


export const GoDropdownSeparator = /*@__PURE__*/ defineContainer<JSX.GoDropdownSeparator>('go-dropdown-separator', defineGoDropdownSeparator);


export const GoField = /*@__PURE__*/ defineContainer<JSX.GoField>('go-field', defineGoField, [
  'idPrefix',
  'controlId',
  'labelId',
  'prefixId',
  'suffixId',
  'hintId',
  'errorId',
  'name',
  'label',
  'disabled',
  'hint',
  'error',
  'readonly',
  'controlElSelector'
]);


export const GoFieldset = /*@__PURE__*/ defineContainer<JSX.GoFieldset>('go-fieldset', defineGoFieldset, [
  'label',
  'hint',
  'error',
  'labelId',
  'hintId',
  'errorId',
  'isList'
]);


export const GoFooter = /*@__PURE__*/ defineContainer<JSX.GoFooter>('go-footer', defineGoFooter, [
  'links',
  'navLabel',
  'dark',
  'navCols',
  'navColsTablet',
  'navColsDesktop'
]);


export const GoGovAuLogo = /*@__PURE__*/ defineContainer<JSX.GoGovAuLogo>('go-gov-au-logo', defineGoGovAuLogo, [
  'height',
  'href'
]);


export const GoHeaderBar = /*@__PURE__*/ defineContainer<JSX.GoHeaderBar>('go-header-bar', defineGoHeaderBar, [
  'breakpoint'
]);


export const GoHeadingRow = /*@__PURE__*/ defineContainer<JSX.GoHeadingRow>('go-heading-row', defineGoHeadingRow, [
  'heading',
  'headingTag',
  'moreLinkHref',
  'moreLinkText'
]);


export const GoHero = /*@__PURE__*/ defineContainer<JSX.GoHero>('go-hero', defineGoHero, [
  'heading',
  'preHeading',
  'breadcrumbs',
  'imgSrc',
  'imgAlt',
  'hideImgOnMobile'
]);


export const GoIcon = /*@__PURE__*/ defineContainer<JSX.GoIcon>('go-icon', defineGoIcon, [
  'iconSet',
  'name',
  'size',
  'color',
  'label'
]);


export const GoInput = /*@__PURE__*/ defineContainer<JSX.GoInput, JSX.GoInput["value"]>('go-input', defineGoInput, [
  'type',
  'labelId',
  'prefixId',
  'suffixId',
  'hintId',
  'errorId',
  'name',
  'label',
  'disabled',
  'hint',
  'error',
  'readonly',
  'value'
],
'value', 'input');


export const GoLink = /*@__PURE__*/ defineContainer<JSX.GoLink>('go-link', defineGoLink, [
  'href',
  'target',
  'expandClickableArea'
]);


export const GoMainNav = /*@__PURE__*/ defineContainer<JSX.GoMainNav>('go-main-nav', defineGoMainNav, [
  'items',
  'label',
  'navigate'
]);


export const GoMd = /*@__PURE__*/ defineContainer<JSX.GoMd>('go-md', defineGoMd, [
  'inline',
  'content',
  'src',
  'mdOptions',
  'sanitise',
  'useGoUi',
  'init',
  'rendered'
]);


export const GoNavDrawer = /*@__PURE__*/ defineContainer<JSX.GoNavDrawer>('go-nav-drawer', defineGoNavDrawer, [
  'position',
  'items',
  'active',
  'label',
  'autoClose',
  'open',
  'close',
  'navItemClick'
]);


export const GoNavLink = /*@__PURE__*/ defineContainer<JSX.GoNavLink>('go-nav-link', defineGoNavLink, [
  'item',
  'showArrow',
  'block',
  'navigate'
]);


export const GoNavList = /*@__PURE__*/ defineContainer<JSX.GoNavList>('go-nav-list', defineGoNavList, [
  'items',
  'headingItem',
  'heading',
  'block',
  'expandSubLists'
]);


export const GoOverlay = /*@__PURE__*/ defineContainer<JSX.GoOverlay>('go-overlay', defineGoOverlay, [
  'active',
  'persistent',
  'overlayOpen',
  'overlayClose'
]);


export const GoProgress = /*@__PURE__*/ defineContainer<JSX.GoProgress>('go-progress', defineGoProgress, [
  'min',
  'max',
  'value',
  'label',
  'labelledby',
  'indeterminate',
  'showPercentage'
]);


export const GoRadio = /*@__PURE__*/ defineContainer<JSX.GoRadio, JSX.GoRadio["value"]>('go-radio', defineGoRadio, [
  'checked',
  'indeterminate',
  'name',
  'disabled',
  'value',
  'label',
  'hint',
  'error',
  'hintId',
  'errorId'
],
'value', 'v-go-change', 'goChange');


export const GoSearchBar = /*@__PURE__*/ defineContainer<JSX.GoSearchBar>('go-search-bar', defineGoSearchBar);


export const GoSelect = /*@__PURE__*/ defineContainer<JSX.GoSelect, JSX.GoSelect["value"]>('go-select', defineGoSelect, [
  'name',
  'label',
  'labelId',
  'suffixId',
  'hintId',
  'errorId',
  'disabled',
  'hint',
  'error',
  'readonly',
  'value',
  'options',
  'goChange'
],
'value', 'v-go-change', 'goChange');


export const GoSkipLink = /*@__PURE__*/ defineContainer<JSX.GoSkipLink>('go-skip-link', defineGoSkipLink, [
  'target'
]);


export const GoSpinner = /*@__PURE__*/ defineContainer<JSX.GoSpinner>('go-spinner', defineGoSpinner, [
  'ringColor',
  'baseColor',
  'size',
  'ringWidth',
  'duration',
  'stacked',
  'loading'
]);


export const GoSwitch = /*@__PURE__*/ defineContainer<JSX.GoSwitch, JSX.GoSwitch["checked"]>('go-switch', defineGoSwitch, [
  'checked',
  'name',
  'label',
  'disabled',
  'value',
  'stack',
  'fullWidth',
  'showOnOff',
  'showOnOffOutside',
  'activeLabel',
  'inactiveLabel'
],
'checked', 'change');


export const GoTab = /*@__PURE__*/ defineContainer<JSX.GoTab>('go-tab', defineGoTab, [
  'label',
  'active',
  'tabId',
  'panelId'
]);


export const GoTableWrapper = /*@__PURE__*/ defineContainer<JSX.GoTableWrapper>('go-table-wrapper', defineGoTableWrapper, [
  'striped',
  'bordered',
  'hoverable'
]);


export const GoTabs = /*@__PURE__*/ defineContainer<JSX.GoTabs>('go-tabs', defineGoTabs, [
  'tabGroupLabel',
  'vertical',
  'manual',
  'tabChange'
]);


export const GoTextarea = /*@__PURE__*/ defineContainer<JSX.GoTextarea, JSX.GoTextarea["value"]>('go-textarea', defineGoTextarea, [
  'labelId',
  'prefixId',
  'suffixId',
  'hintId',
  'errorId',
  'name',
  'label',
  'disabled',
  'hint',
  'error',
  'readonly',
  'value',
  'autoGrow'
],
'value', 'input');


export const GoToTop = /*@__PURE__*/ defineContainer<JSX.GoToTop>('go-to-top', defineGoToTop, [
  'offset'
]);


export const GoToc = /*@__PURE__*/ defineContainer<JSX.GoToc>('go-toc', defineGoToc, [
  'label',
  'selector',
  'labelClass'
]);


export const GoTooltip = /*@__PURE__*/ defineContainer<JSX.GoTooltip>('go-tooltip', defineGoTooltip, [
  'triggerId',
  'arrow',
  'inline',
  'placement'
]);

