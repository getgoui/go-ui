...
<div className="text-size-1">Link is a fundamental component of HTML, it is important to get this right.</div>
...
It should be noted that [links to cross-origin destinations are unsafe](https://web.dev/external-anchors-use-rel-noopener/), so whenever you need to link to an external site, you should add `rel="noopener noreferrer"` to the link, `go-link` will take care of this for you when it detects the link is external.
