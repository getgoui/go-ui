---
title: Nav list
hide_title: true
hide_table_of_contents: true
sidebar_position: 2
---

import Demo from '@/components/Demo';
import demoSource from '!!raw-loader!@/go-ui/components/navigation/go-nav-list/demo/go-nav-list.html';

# Nav list <span className="text-size-0">`go-nav-list`</span>

<!-- Description -->

<div className="text-size-1">Navigation list is a group of nav links in the native nav landmark.</div>

A navigation list can have multiple groups inside.

## Accessibility

`go-nav-list` will render the native `nav` element which tells assistive technologies that it has a aria role of `navigation`.

:::tip
It is important to distinguish each navigation list by providing a unique label for each list.
:::

## Related patterns

<!-- Patterns that uses this component -->

- [Footer](#)
- [Sidebar nav](#)

<!-- Demos, tips, variations, use cases -->

## Demo

<Demo code={demoSource} />

<!-- API -->

{@include: ../../../../src/components/navigation/go-nav-list/readme.md}
