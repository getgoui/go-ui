...
The scale factor we chose as default is `1.25`. This means starting from the base font-size (16px/1rem), each step goes up (or down) by 25%.

The default sizes from the calculator:
![Type scales table](/img/docs/typography-scales.png)

There are 9 scale steps in total:
| Scale step | Description | Utility class |
|------------|-------------|----------------|
| -1 | The smallest _readable_ font size | `.text-size--1` `.text-small` |
| 0 | The base font size used in body (also used by `h6`) | `.text-size-0` `.text-body` |
| 1 | A slightly bigger size to provide emphasis (also used by `h5`) | `.text-size-1` `.text-big` |
...
| 7 | Largest font size on site (e.g. landing page hero heading) | `.text-size-7` |

#### Type scale utility classes

You can use the _`.text-size-{n}`_ utility classes to set the text scale to a specific level (-1 to 7).
